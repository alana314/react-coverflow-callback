...
class Coverflow extends Component {
    ...
    setFigureRef = (index, node) => {
        this.figureRefs[index] = node
    }
    ...
    componentDidMount() {
        ...
        TRANSITIONS.forEach((event) => {
            for (let i = 0; i < length; i++) {
                const figureRef = this.figureRefs[i] // Access the ref from the object
                if (figureRef) {
                    figureRef.addEventListener(
                        event,
            ...
            <div ref={this.refNode}>
                <StyleRoot>
                    <div
                        ...
                        onWheel={
                            enableScroll ? this._handleWheel.bind(this) : null
                        }
                    ...
                    >
                        <div className={styles.coverflow}>
                            ...
                            <div className={styles.stage} ref={this.stageRef}>
                                {navigation && (
                                    <div
                                        id={styles.arrow1}
    ...
    _handleFigureStyle(index, current) {
        ...
        if (index === current) {
            ...
            }px)  scale(${this.props.currentFigureScale}`
            ...
            style.transform = `translateX(${
                this.state.move + offset
            }px) rotateY(40deg) scale(${this.props.otherFigureScale}`
            style.zIndex = `${10 - depth}`
            style.opacity = opacity
            if (navigation) {
            ...
            (child, index) => {
                ...
                return (
                    <figure
                        ...
                        style={style}
                        ref={(node) => this.setFigureRef(index, node)}
                    >
                        {figureElement}
                        {enableHeading && (
    ...
    _handlePrevFigure = (e) => {
        ...
        if (current - 1 < 0 && infiniteScroll) {
            ...
            this.props.onChange(this.props.children.length - 1)
            TOUCH.lastMove = move
        }
    }
    ...
    // Debounce the _handleNextFigure method
    _handleNextFigureDebounced = debounce(this._handleNextFigure, 300)

    _handlePrevFigureDebounced = debounce(this._handlePrevFigure, 300)
